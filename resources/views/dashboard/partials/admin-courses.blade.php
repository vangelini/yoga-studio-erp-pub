...
<div class="card p-6 space-y-8" x-data="{ showCreateCourse: false }">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
        <div class="space-y-1">
            <h3 class="text-2xl font-semibold text-green-800">{{ $courseCardTitle }}</h3>
    ...
    <div
        ...
        x-data="{
            ...
            pricingMode: '{{ old('pricing_mode', 'block') }}',
        ...
        <form method="POST" action="{{ route('admin.courses.store') }}" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div class="space-y-3">
                <div class="space-y-1.5">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required class="input-field text-sm" placeholder="Titolo del corso">
                    @error('title')
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="flex flex-wrap items-center gap-4 text-sm text-stone-600">
                        <label class="inline-flex items-center gap-2">
                            <input type="radio" name="pricing_mode" value="block" class="h-4 w-4 border-stone-300 text-teal-600" x-model="pricingMode" @checked(old('pricing_mode', 'block') === 'block')>
                            <span>Prezzo a blocco (mensile/trimestrale/annuale)</span>
                        </label>
                        <label class="inline-flex items-center gap-2">
                            <input type="radio" name="pricing_mode" value="per_lesson" class="h-4 w-4 border-stone-300 text-teal-600" x-model="pricingMode" @checked(old('pricing_mode') === 'per_lesson')>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-2" x-show="pricingMode === 'block'" x-cloak>
                        <label class="flex flex-col gap-1 text-sm text-stone-600 w-full">
                            ...
                            <input type="number" step="0.01" name="monthly_price" class="input-field text-sm w-full" value="{{ old('monthly_price') }}">
                        </label>
                        <label class="flex flex-col gap-1 text-sm text-stone-600 w-full">
                            <span class="text-xs uppercase font-semibold text-stone-500 block">Trimestrale</span>
                            <input type="number" step="0.01" name="quarterly_price" class="input-field text-sm w-full" value="{{ old('quarterly_price') }}">
                        </label>
                        <label class="flex flex-col gap-1 text-sm text-stone-600 w-full">
                            <span class="text-xs uppercase font-semibold text-stone-500 block">Annuale</span>
                            <input type="number" step="0.01" name="annual_price" class="input-field text-sm w-full" value="{{ old('annual_price') }}">
                        </label>
                    </div>
                    <div class="space-y-2" x-show="pricingMode === 'per_lesson'" x-cloak>
                        @foreach ($planOptions as $planKey => $planLabel)
                            <div class="rounded-lg border border-stone-200 bg-white px-3 py-2">
                                <p class="text-xs font-semibold text-stone-600">{{ $planLabel }}</p>
                                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
                                    @for ($lessons = 1; $lessons <= 4; $lessons++)
                                        <label class="text-[11px] text-stone-500 space-y-1" x-show="{{ $lessons }} <= maxLessonOptions()" x-cloak>
                                            <span>{{ $lessons }} lezioni/settimana</span>
                                            <input
                                                type="number"
                                                step="0.01"
                                                min="0"
                                                name="lesson_pricing[{{ $planKey }}][{{ $lessons }}]"
                                                value="{{ old('lesson_pricing.'.$planKey.'.'.$lessons) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" min="1" name="max_enrollments" value="{{ old('max_enrollments') }}" class="input-field text-sm" placeholder="Es. 20">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div class="space-y-1.5">
                        ...
                        <input type="date" name="start_date" value="{{ old('start_date') }}" required class="input-field text-sm">
                        @error('start_date')
                            <p class="text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                    <div class="space-y-1.5">
                        ...
                        <input type="date" name="end_date" value="{{ old('end_date') }}" required class="input-field text-sm">
                        @error('end_date')
                            <p class="text-xs text-rose-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <input type="number" step="0.1" min="0" max="100" name="extra_day_discount_percent" value="{{ old('extra_day_discount_percent', 0) }}" class="input-field text-sm" placeholder="Es. 10">
                    <p class="text-[11px] text-stone-500">Percentuale applicata sul costo extra prima della riduzione per lezioni rimanenti.</p>
                    @error('extra_day_discount_percent')
                        <p class="text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
            </div>
            ...
            <div class="space-y-3">
                <div class="space-y-1.5">
                    ...
                    <textarea name="description" rows="5" class="input-field text-sm" placeholder="Descrizione sintetica del corso" required>{{ old('description') }}</textarea>
                    @error('description')
                        <p class="text-xs text-rose-600">{{ $message }}</p>
                    @enderror
                </div>
        ...
        @forelse ($courses as $course)
            <div
                ...
                x-data="{
                    ...
                    schedule: {{ Js::from($course['schedule']) }}.length ? {{ Js::from($course['schedule']) }} : [{ day: '', time: '', capacity: '' }],
                    addSlot() { this.schedule.push({ day: '', time: '', capacity: '' }); },
                    removeSlot(index) { if (this.schedule.length > 1) this.schedule.splice(index, 1); },
                    filledSlots() { return this.schedule.filter(slot => slot.day && slot.time).length || 0; },
                    maxLessonOptions() { return Math.min(4, this.filledSlots()); }
            ...
            >
                <div class="flex flex-wrap items-center gap-3 text-sm text-stone-600">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:gap-2 w-full">
                        <span class="text-xl font-semibold text-green-800">{{ $course['title'] }}</span>
                    ...
                    <div class=" items-center gap-2 text-xs text-stone-500">
                        ...
                        <span>
                            {{ $course['teacher_name'] ?? ($isTeacherView ? $currentTeacherName : 'Non assegnato') }}
                        </span>
                    </div>
                    ...
                    <div class="flex flex-wrap items-center gap-1 text-xs text-stone-500">
                        ...
                        @endif
                        @if(!empty($course['max_enrollments']))
                            <span class="inline-flex items-center gap-1 rounded-full bg-stone-100 px-2 py-0.5 font-semibold text-stone-500">
                                Capienza max: {{ $course['max_enrollments'] }} allievi
                            </span>
                        @endif
                    </div>
                ...
                <div class="mt-6 space-y-3" x-data="{ showStudents: false }">
                    ...
                    <div>
                        <button type="button" class=" flex justify-end items-center gap-2 rounded-lg border border-stone-300 px-3 py-1.5 text-xs font-semibold text-stone-600 hover:bg-stone-100 transition" @click="showStudents = !showStudents">
                            ...
                            <span x-show="!showStudents">Mostra Allieve/i iscritti (num. {{ count($course['students'] ?? []) }})</span>
                    ...
                    <div class="space-y-3" x-show="showStudents" x-transition>
                        @foreach ($course['students'] as $student)
                            <div class="flex flex-col gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 shadow-sm md:flex-row md:items-center md:justify-between">
                                <div>
                                    <a href="{{ route('admin.clients.index', ['client_id' => $student['client_id']]) }}#client-{{ $student['client_id'] }}" class="text-sm font-semibold text-teal-700 hover:text-teal-900">
                                        {{ $student['name'] }}
                                    </a>
                                ...
                                <div class="text-xs text-stone-500 space-y-1">
                                    @if (!empty($student['email']))
                                        <a href="mailto:{{ $student['email'] }}" class="inline-flex items-center gap-1 text-teal-600 font-semibold hover:text-teal-800">
                                    ...
                                    @if (!empty($student['telephone']))
                                        <div class="flex items-center gap-2">
                                            ...
                                            @if (!empty($student['whatsapp']))
                                                <a href="{{ $student['whatsapp'] }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 text-emerald-600 font-semibold hover:text-emerald-800">
                                                    {{ $student['telephone'] }}
                                                </a>
                                            @else
                    ...
                    <form method="POST" action="{{ route('admin.courses.update', $course['id']) }}" class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="flex flex-wrap items-center gap-4 text-sm text-stone-600">
                                    <label class="inline-flex items-center gap-2">
                                        <input type="radio" name="pricing_mode" value="block" class="h-4 w-4 border-stone-300 text-teal-600" x-model="pricingMode" @checked(($course['pricing_mode'] ?? 'block') === 'block')>
                                        <span>Prezzo a blocco</span>
                                    </label>
                                    <label class="inline-flex items-center gap-2">
                                        <input type="radio" name="pricing_mode" value="per_lesson" class="h-4 w-4 border-stone-300 text-teal-600" x-model="pricingMode" @checked(($course['pricing_mode'] ?? 'block') === 'per_lesson')>
                            ...
                            <div class="space-y-1.5">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2" x-show="pricingMode === 'block'" x-cloak>
                                    <label class="flex flex-col gap-1 text-sm text-stone-600 w-full">
                                        <span class="text-xs uppercase font-semibold text-stone-500 block">Mensile</span>
                                        <input type="number" step="0.01" name="monthly_price" value="{{ number_format($course['monthly_price'] ?? $course['price'] ?? 0, 2, '.', '') }}" class="input-field text-sm w-full" placeholder="Mensile">
                                    </label>
                                    <label class="flex flex-col gap-1 text-sm text-stone-600 w-full">
                                        <span class="text-xs uppercase font-semibold text-stone-500 block">Trimestrale</span>
                                        <input type="number" step="0.01" name="quarterly_price" value="{{ number_format($course['quarterly_price'] ?? 0, 2, '.', '') }}" class="input-field text-sm w-full" placeholder="Trimestrale">
                                    </label>
                                    <label class="flex flex-col gap-1 text-sm text-stone-600 w-full">
                                        <span class="text-xs uppercase font-semibold text-stone-500 block">Annuale</span>
                                        <input type="number" step="0.01" name="annual_price" value="{{ number_format($course['annual_price'] ?? 0, 2, '.', '') }}" class="input-field text-sm w-full" placeholder="Annuale">
                                    </label>
                                </div>
                                <div class="space-y-2" x-show="pricingMode === 'per_lesson'" x-cloak>
                                    @foreach ($planOptions as $planKey => $planLabel)
                                        @php $planPricing = $course['lesson_pricing'][$planKey] ?? []; @endphp
                                        <div class="rounded-lg border border-stone-200 bg-white px-3 py-2">
                                            <p class="text-xs font-semibold text-stone-600">{{ $planLabel }}</p>
                                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2">
                                                @for ($lessons = 1; $lessons <= 4; $lessons++)
                            ...
                            <div class="space-y-1.5">
                                <label class="text-xs uppercase font-semibold text-stone-500">Capienza massima corso</label>
                                <input type="number" min="1" name="max_enrollments" value="{{ $course['max_enrollments'] }}" class="input-field text-sm" placeholder="Es. 20">
                            </div>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                <div class="space-y-1.5">
                                    <label class="text-xs uppercase font-semibold text-stone-500">Data inizio <span class="text-rose-600">*</span></label>
                            ...
                            <div class="flex items-start gap-3 rounded-xl border border-teal-100 bg-white px-3 py-2">
                                <input type="checkbox" name="allows_extra_day" value="1" class="mt-1 h-4 w-4 rounded border-stone-300 text-teal-600 focus:ring-teal-500" @checked($course['allows_extra_day'] ?? false)>
                                <div>
                                    <p class="text-sm font-semibold text-stone-700">Disponibile come "Un giorno in piú"</p>
                            ...
                            <div class="space-y-1.5">
                                <label class="text-xs uppercase font-semibold text-stone-500">Sconto "Un giorno in piú" (%)</label>
                                <input type="number" step="0.1" min="0" max="100" name="extra_day_discount_percent" value="{{ number_format($course['extra_day_discount_percent'] ?? 0, 1, '.', '') }}" class="input-field text-sm">
                        ...
                        <div class="space-y-3">
                            <div class="space-y-1.5">
                                <label class="text-xs uppercase font-semibold text-stone-500">Descrizione <span class="text-rose-600">*</span></label>
                                <textarea name="description" rows="4" class="input-field text-sm" required>{{ $course['description'] }}</textarea>
                                @error('description')
                                    <p class="text-xs text-rose-600">{{ $message }}</p>
                                @enderror
                            </div>
